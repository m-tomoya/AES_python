Sbox = (
    ,
)

InvSbox = (
    ,
)

Rcon = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
    0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
)


def xtime(a): return (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)


def text2matrix(text):
    matrix = []
    for i in range(16):
        byte = (text >> (8 * (15 - i))) & 0xFF
        if i % 4 == 0:
            matrix.append([byte])
        else:
            matrix[i // 4].append(byte)
    return matrix


def matrix2text(matrix):
    text = 0
    for i in range(4):
        for j in range(4):
            text |= (matrix[i][j] << (120 - 8 * (4 * i + j)))
    return text


class AES:
    def __init__(self, master_key):
        self.key_expansion(master_key)

    def debug(self, list):
        debug = []
        for m in range(len(list)):
            # print(hex(list[m]))
            debug.append(hex(list[m]))
        print(debug)

    def sub_word(self, word):
        for i in range(len(word)):
            word[i] = Sbox[word[i]]
        return word

    def rot_word(self, word):
        old = list(word)
        word[0] = old[1]
        word[1] = old[2]
        word[2] = old[3]
        word[3] = old[0]
        return word

    def key_expansion(self, master_key):
        self.round_keys = text2matrix(master_key)
        word = list([] for i in range(44))
        for i in range(4):
            word[i] = self.round_keys[i]
        for i in range(4, 4*11):
            temp = list(word[i-1])
            if i % 4 == 0:
                rcon = []
                rcon.append(Rcon[i//4])
                for _ in range(3):
                    rcon.append(0x00)
                temp = self.sub_word(self.rot_word(temp))
                for j in range(len(temp)):
                    temp[j] = temp[j] ^ rcon[j]
            for k in range(len(temp)):
                t = word[i-4][k] ^ temp[k]
                word[i].append(t)
            self.round_keys = word

    def add_round_key(self, state, key):
        for i in range(4):
            for j in range(4):
                state[i][j] ^= key[i][j]

    def sub_bytes(self, state):
        for i in range(4):
            for j in range(4):
                state[i][j] = Sbox[state[i][j]]

    def inv_sub_bytes(self, s):
        for i in range(4):
            for j in range(4):
                s[i][j] = InvSbox[s[i][j]]

    def shift_rows(self, s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[1][1], s[2][1], s[3][1], s[0][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[3][3], s[0][3], s[1][3], s[2][3]

    def inv_shift_rows(self, s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[3][1], s[0][1], s[1][1], s[2][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[1][3], s[2][3], s[3][3], s[0][3]

    def __mix_single_column(self, a):
        # please see Sec 4.1.2 in The Design of Rijndael
        t = a[0] ^ a[1] ^ a[2] ^ a[3]
        u = a[0]
        a[0] ^= t ^ xtime(a[0] ^ a[1])
        a[1] ^= t ^ xtime(a[1] ^ a[2])
        a[2] ^= t ^ xtime(a[2] ^ a[3])
        a[3] ^= t ^ xtime(a[3] ^ u)

    def mix_columns(self, s):
        for i in range(4):
            self.__mix_single_column(s[i])

    def inv_mix_columns(self, s):
        # see Sec 4.1.3 in The Design of Rijndael
        for i in range(4):
            u = xtime(xtime(s[i][0] ^ s[i][2]))
            v = xtime(xtime(s[i][1] ^ s[i][3]))
            s[i][0] ^= u
            s[i][1] ^= v
            s[i][2] ^= u
            s[i][3] ^= v

        self.mix_columns(s)

    def round_encrypt(self, round, state, key):
        self.sub_bytes(state)
        print("round[{}].s_box {}".format(
            round, hex(matrix2text(self.plain_state))))
        self.shift_rows(state)
        print("round[{}].s_row {}".format(
            round, hex(matrix2text(self.plain_state))))
        self.mix_columns(state)
        print("round[{}].s_row {}".format(
            round, hex(matrix2text(self.plain_state))))
        self.add_round_key(state, key)
        print("round[{}].k_sch {}".format(
            round, hex(matrix2text(key))))

    def round_decrypt(self, round, state, key):
        self.inv_shift_rows(state)
        print("round[{}].is_row {}".format(
            round, hex(matrix2text(self.cipher_state))))

        self.inv_sub_bytes(state)
        print("round[{}].is_box {}".format(
            round, hex(matrix2text(self.cipher_state))))

        self.add_round_key(state, key)
        print("round[{}].ik_sch {}".format(
            round, hex(matrix2text(key))))

        self.inv_mix_columns(state)
        print("round[{}].im_col {}".format(
            round, hex(matrix2text(self.cipher_state))))

    def encrypt(self, plain_text):
        self.plain_state = text2matrix(plain_text)
        print("round[0].input {}".format(hex(matrix2text(self.plain_state))))

        round_key = self.round_keys[:4]
        self.add_round_key(self.plain_state, round_key)
        print("round[0].k_sch {}".format(
            hex(matrix2text(round_key))))

        for i in range(1, 10):
            print("round[{}].start {}".format(
                i, hex(matrix2text(self.plain_state)))
            )
            self.round_encrypt(i, self.plain_state,
                               self.round_keys[4 * i: 4 * (i + 1)]
                               )
        print("round[10].start {}".format(hex(matrix2text(self.plain_state))))

        self.sub_bytes(self.plain_state)
        print("round[10].s_box {}".format(hex(matrix2text(self.plain_state))))

        self.shift_rows(self.plain_state)
        print("round[10].s_row {}".format(hex(matrix2text(self.plain_state))))

        round_key = self.round_keys[40:]
        self.add_round_key(self.plain_state, round_key)
        print("round[10].k_sch {}".format(hex(matrix2text(round_key))))

        out_put = matrix2text(self.plain_state)
        print("round[10].output {}".format(hex(out_put)))

        return out_put

    def decrypt(self, ciphertext):
        self.cipher_state = text2matrix(ciphertext)
        print("round[0].iinput {}".format(hex(matrix2text(self.cipher_state))))

        round_key = self.round_keys[40:]
        self.add_round_key(self.cipher_state, round_key)
        print("round[0].ik_sch {}".format(
            hex(matrix2text(round_key))))

        for i in range(9, 0, -1):
            round = 10-i
            print("round[{}].istart {}".format(
                round, hex(matrix2text(self.cipher_state)))
            )
            self.round_decrypt(
                round, self.cipher_state, self.round_keys[4 * i: 4 * (i + 1)])

        print("round[10].istart {}".format(
            hex(matrix2text(self.cipher_state))))

        self.inv_shift_rows(self.cipher_state)
        print("round[10].is_row {}".format(
            hex(matrix2text(self.cipher_state))))

        self.inv_sub_bytes(self.cipher_state)
        print("round[10].is_box {}".format(
            hex(matrix2text(self.cipher_state))))

        round_key = self.round_keys[:4]
        self.add_round_key(self.cipher_state, round_key)
        print("round[10].k_sch {}".format(hex(matrix2text(round_key))))

        out_put = matrix2text(self.cipher_state)
        print("round[10].ioutput {}".format(hex(out_put)))
        return out_put


master_key = 0x000102030405060708090a0b0c0d0e0f
plain_text = 0x00112233445566778899aabbccddeeff
print("PLAINTEXT: {}".format(hex(plain_text)))
print("KEY: {}".format(hex(master_key)))
print("CIPHER (ENCRYPT): ")
aes = AES(master_key)
cipher_text = aes.encrypt(plain_text)


print("====================================================")
print("CIPHERTEXT: {}".format(hex(cipher_text)))
print("KEY: {}".format(hex(master_key)))
print("CIPHER (DECRYPT): ")
decrypted_text = aes.decrypt(cipher_text)
